...
while True:
    ...
    # Add a sickness to an animal
        ...
        if is_current == 'y':
            ...
            date_of_event = input(
                'Please enter when the event took place (YYYY-MM-DD): '
            )
            animals_dict[id_to_modify].add_event(
                Actions.sick.value, sickness_name, date_of_event
            )
        else:
            print('Please enter a valid answer (y/n)')
        print(
            f'Added sickness ({sickness_name}) to animal called {animals_dict[id_to_modify].name}'
        )

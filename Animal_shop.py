...
from Actions_enum import print_menu
from Actions_enum import enter_to_continue
...
while True:
    ...
    try:
        ...
        # Sell an animal
            ...
            are_you_sure = input(
                f'Are you sure you want to remove {animals_dict[animal_id].name} from the database (y/n): '
            )
            if are_you_sure == 'y':
        ...
        # Get history of the animal
            ...
            history_type = int(
                input(
                    f'Please enter the type of history: {Actions.sick.value} for sicknesses, {Actions.operation.value} for operations and {Actions.vaccine.value} for vaccinations: '
                )
            )
        ...
        # Add a sickness to an animal
            ...
            animal_id = int(input('Please enter an animal ID to register sickness to: '))
            sickness_name = input('Please enter the name of the sickness: ')
            is_current = input('Is this a current sickness? y/n: ')

            if is_current == 'y':
                animals_dict[animal_id].add_event(Actions.sick.value, sickness_name)
                animals_dict[animal_id].cond = 'Sick'
                print(f'Added sickness ({sickness_name}) to animal called {animals_dict[animal_id].name}')   
            elif is_current == 'n':
                date_of_event = input('Please enter when the event took place (YYYY-MM-DD): ')
                animals_dict[animal_id].add_event(Actions.sick.value, sickness_name, date_of_event)
                print(f'Added sickness ({sickness_name}) to animal called {animals_dict[animal_id].name}')   
            else:
        ...
        # Add operation to an animal
            ...
            animal_id = int(
                input('Please enter an animal ID to register operation to: ')
            )
            operation_name = input('Please enter the name of the operation: ')
            date_of_event = input('Please enter when the event took place (YYYY-MM-DD): ')

            # Add operation to object
            animals_dict[animal_id].add_event(
                Actions.operation.value, operation_name, date_of_event
            )
            print(
                f'Added operation ({operation_name}) to animal called {animals_dict[animal_id].name}'
            )
            enter_to_continue()
        ...
        # Add vaccination to an animal
            ...
            animal_id = int(
                input('Please enter an animal ID to register vaccination to: ')
            )
            ...
            animals_dict[animal_id].add_event(
                Actions.vaccine.value, vaccine_name, date_of_event
            )
            print(
                f'Added vaccine ({vaccine_name}) to animal called {animals_dict[animal_id].name}'
            )
            enter_to_continue()
        ...
        print(f'An animal with the ID ({animal_id}) cannot be found!')
        enter_to_continue()
